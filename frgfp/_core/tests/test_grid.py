"""
Unit tests for the frgfp core collocation grid generators.
"""
...
def test_uniform_collgrid_1d():
    """Check a 1D uniform grid and its ordering."""
...
def test_uniform_collgrid_multidim():
    """Check a 2D uniform grid extent and lexicographic ordering."""
...
def test_chebyshev_collgrid_1d():
    """Check the 1D Chebyshev-Gauss-Lobatto nodes on [-1, 1]."""
...
def test_chebyshev_collgrid_boundaries():
    """Check that 2D Chebyshev nodes span the requested intervals."""
...
def test_grid_input_validation():
    """Check that invalid inv_dim/num_points/interval inputs raise clear errors."""
    ...
    with pytest.raises(ValueError):
        fp.chebyshev_collgrid(inv_dim=1, num_points=1, interval=(0.0, 1.0)) # need at least 2 points